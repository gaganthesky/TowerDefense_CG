//To load OBJ in the project, used this code from web using this tutorial: vimeo.com/38898017
//Change struct values suiting my OBJ
...
typedef struct
{
  ...
  float Ka[4],Kd[4],Ks[4],Ke[4],Tf[4],Ns, Ni; /*  Colors and shininess */
...
static void loadMaterial(char* file)
{
  ...
  while ((line = readline(f))) {
    ...
    if ((str = readstr(line,"newmtl"))) {
      ...
      mtl[k].Tf[0] = mtl[k].Tf[1] = mtl[k].Tf[2] = 0;   mtl[k].Tf[3] = 1;
      mtl[k].Ns  = 0;
      mtl[k].Ni  = 0;
      mtl[k].d   = 0;
      mtl[k].map = 0;
...
static void setMaterial(char* name)
{
  ...
  for (k=0;k<Nmtl;k++)
    if (!strcmp(mtl[k].name,name)) {
      ...
      glMaterialfv(GL_FRONT_AND_BACK,GL_EMISSION,mtl[k].Tf);
      glMaterialfv(GL_FRONT_AND_BACK,GL_EMISSION,mtl[k].Ke);
      glMaterialfv(GL_FRONT_AND_BACK,GL_COLOR_INDEXES,&mtl[k].Ni);
